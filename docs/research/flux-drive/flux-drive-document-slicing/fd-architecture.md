...
- P1 | ARCH-001 | "F0: Interserve MCP Server" | New MCP server duplicates existing dispatch infrastructure
...
- P3 | ARCH-005 | "F0: Interserve MCP Server" | Systemd service for classification is premature optimization
...
3. **Shared library extraction:** Move tier resolution into a Go package (`pkg/tiers`) consumed by both interlock-mcp and interserve-mcp. Then both servers inherit tier changes automatically.
...
**Conflict:** The PRD proposes Interserve spark semantic classification, but `slicing.md` specifies deterministic keyword matching. These are different algorithms with different output characteristics (semantic vs syntactic, probabilistic vs deterministic). The PRD does not address which is authoritative or how they compose.

**Architectural question:** Is Interserve spark classification replacing the keyword-based algorithm in `slicing.md`, or augmenting it? If replacing, `slicing.md` needs a major rewrite. If augmenting, you need a composition rule (e.g., "spark classification with keyword-based fallback for low-confidence sections").

**Recommendation:** Make the relationship explicit:
1. **Option A (replacement):** Interserve spark IS the classification algorithm. Rewrite `slicing.md` Section Classification to say "Invoke interserve MCP `classify_sections` tool with agent domain keywords." Move keyword lists into the MCP prompt as classification hints.
2. **Option B (fallback chain):** Try Interserve spark first; if unavailable or low-confidence, fall back to keyword matching from `slicing.md`. Document the confidence threshold and fallback trigger in `slicing.md`.
3. **Option C (composition):** Interserve spark does initial section grouping, keyword matching validates/refines. This is complex and likely unnecessary.
...
1. The interserve MCP server (to extract section previews for classification)
...
**Location:** F0: Interserve MCP Server acceptance criteria
...
**When systemd makes sense:** If interserve grows to handle other frequent tasks (summary extraction, complexity routing per Open Questions), then always-on makes sense. But for a single low-frequency tool, it's over-architected.
...
- PRD F0: "Graceful degradation: if Interserve spark is unreachable, returns error (caller decides fallback)"
...
The existing `slicing.md` is marked as "single source of truth for all slicing logic." If Interserve spark classification is replacing keyword matching, rewrite `slicing.md` to specify the new algorithm. If it's augmenting, define the composition rule explicitly.

**Concrete change:** Add a new section to `slicing.md`:
```markdown
## Classification Methods

### Method 1: Semantic (Interserve Spark) — preferred when available
Invokes interserve MCP `classify_sections` tool with agent domain keywords.
...
Used when Interserve spark unavailable or returns low-confidence (<0.6).
```
...
Architectural answer: New plugin (`plugins/interserve/`) is correct. Classification is infrastructure (reusable by interpath, interdoc, future routing), not a flux-drive-specific concern. Embedding in interflux would couple classification to the review engine.
...
Fix these boundary issues before implementation. The core idea (semantic classification via Interserve spark) is sound and well-justified by the brainstorm experiments.
