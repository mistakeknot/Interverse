# Flux-Drive Architecture Review: clodex MCP Server

**Reviewed**: 2026-02-16
**Reviewer**: fd-architecture (Flux-Drive)
**Scope**: New clodex Go MCP server + integration with interflux flux-drive system

---

## Executive Summary

**Verdict**: Safe to proceed. The clodex architecture is clean, well-bounded, and correctly integrated.
...
```
cmd/clodex-mcp/          → Entry point (9 lines)
internal/
...
| `classify` | Codex spark classification | `ClassifyResult`, `Classify()`, `AgentDomain` | `extract` |
...
- The MCP server is consumed by interflux (same monorepo), which sets `CLODEX_DISPATCH_PATH` in `plugin.json`.
...
**Risk**: If clodex is ever extracted to a standalone project, the fallback path becomes invalid. However, this is a **future extraction concern**, not a current architecture issue. The env var override provides the proper abstraction for external use.

**Recommendation**: Document the monorepo assumption in `cmd/clodex-mcp/main.go` inline comment:
```go
// Fallback for Interverse monorepo deployment.
// External users should always set CLODEX_DISPATCH_PATH.
```
...
- This is the correct division: clodex **classifies**, flux-drive **orchestrates**.

**Data flow**:
```
flux-drive (orchestrator)
  → mcp__plugin_clodex_clodex__classify_sections(file_path)
  → clodex MCP server (read file + extract + classify via dispatch.sh)
...
**Concern**: The `tempfiles` package exists in clodex but is **not exposed via MCP tools**. This means:
- `tempfiles.GenerateAgentFiles()` is only usable by Go code that imports clodex as a library.
- The flux-drive orchestrator (which invokes clodex via MCP) must reimplement the temp file generation logic.
...
**Mitigation**: Check `launch.md` Step 2.1c. If it says "invoke clodex tempfiles generation," then MCP tool 3 is missing. If it says "flux-drive writes temp files using slicing_map," then `tempfiles` package should be deleted.
...
```go
promptFile, _ := os.CreateTemp("", "clodex-prompt-*.txt")
...
**Boundary**: Clodex does **not** know about Codex spark internals. It only knows:
...
**Concern**: Tight coupling to Clavain's dispatch.sh implementation. If dispatch.sh changes its flag syntax or response format, clodex breaks.

**Mitigation**: The coupling is **intentional and documented**. Clodex is a Clavain companion plugin, not a standalone tool. The README should make this explicit.
...
**Code fence stripping**: `stripCodeFences()` handles LLM output that wraps JSON in markdown fences. This is a **defensive pattern** — Codex spark should return raw JSON, but stripping fences makes clodex resilient to LLM output formatting quirks.
...
**Rationale**: If Codex spark misclassifies the document (e.g., classifies a Go API design doc using game-design agents), the result will be diffuse low-priority assignments across all agents. The 10% threshold detects this and triggers fallback.
...
**Alternative pattern**: Return confidence scores per agent in the JSON response, let orchestrator decide threshold policy. This decouples the heuristic from clodex.
...
**Concern**: The 80% threshold is **hardcoded** in `buildResult()`. If flux-drive wants to tune this threshold per project (e.g., lower threshold for very large documents), it cannot override clodex's logic.
...
```go
// Test that clodex classification matches examples in slicing.md
...
1. Classify sections: Invoke clodex MCP classify_sections tool with file_path.
...
**Analysis**:
- Step 1 invokes clodex MCP tool. ✅
- Step 2 checks `status` field from ClassifyResult. ✅
- Step 3 **does not invoke clodex** — flux-drive writes temp files itself. ⚠️
- Step 4 records paths for later use in agent prompts. ✅

**Conclusion**: The `tempfiles` package in clodex is **orphaned**. It implements the per-agent temp file logic, but the flux-drive orchestrator does not call it.
...
2. **Delete tempfiles package**: Flux-drive owns temp file generation. Clodex only provides classification metadata.
...
- Clodex returning file paths would require clodex to manage temp file lifecycle (when to delete?), which crosses the boundary.

**Better division of responsibility**:
- Clodex: "Here's the slicing metadata (which sections each agent should see)."
...
Or, define cross-cutting agents in `slicing.md` machine-readable format (YAML frontmatter?) and have both clodex and flux-drive read from the same source.
...
**Alternative**: If dispatch.sh accepted stdin for prompts, clodex could use:
```go
cmd.Stdin = strings.NewReader(prompt)
output, _ := cmd.Output()
```

**Verdict**: Temp file approach is **imposed by dispatch.sh's interface**, not clodex's choice. Not accidental complexity.
...
- **Partial output**: If dispatch.sh writes incomplete JSON (e.g., crashes mid-write), `json.Unmarshal()` fails, but clodex doesn't log the raw output for debugging.
...
if err := json.Unmarshal(payload, &decoded); err != nil {
    fmt.Fprintf(os.Stderr, "clodex: invalid JSON from dispatch: %s\n", payload)
...
**Missing**:
- **AGENTS.md**: No developer guide for clodex (how to build, test, extend).
...
- Build instructions (`go build -o bin/clodex-mcp ./cmd/clodex-mcp/`).
- Test instructions (`go test ./... -v`).
- MCP tool reference (tool names, parameters, return schemas).
- Integration with flux-drive (when to use clodex vs. keyword-based slicing).
...
- **Rationale**: Decouple agent roster from clodex code. Easier to add new cross-cutting agents without code changes.
...
**Risk**: If Clavain's `dispatch.sh` changes its interface (flags, JSON schema, error handling), clodex breaks.
...
- Test fails on dispatch.sh interface changes, forcing explicit clodex update.
...
**Risk**: `slicing.md` spec diverges from clodex implementation over time.
...
- [ ] **Invoke clodex MCP tool** `classify_sections` when document >= 200 lines.
...
- [ ] **Write per-agent temp files** (flux-drive's responsibility, not clodex's).
- [ ] **Handle cross-cutting agents**: Skip slicing for agents in `CrossCuttingAgents` map (or get list from clodex response).
...
The clodex MCP server is architecturally sound. Module boundaries are clear, coupling is intentional and directional, and integration with flux-drive is well-specified. The only blocking issue is the orphaned `tempfiles` package, which should be resolved before shipping.
...
**Edge weights**:
- `cmd/clodex-mcp` → `tools`: Bootstrap (thin, stateless).
